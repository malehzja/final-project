# final-project
The copendium contains the data and notebooks associated with the final project. Each notebook corresponds to the task asked to accomplish, 2 text analysis methods learnt in LIS 4/5683 and 1 machine learning model. The methods chosen were Sentiment Analysis and Descriptive Analysis and Visualization using Altair. It is organizaed as follows:
* The final-project\ folder contains the workflow for the analysis of the final project:
*   The DescriptiveAnalysis.ipynb notebook contains the code used to make basic and advanced viusals using Altair
*   The MachineLearning.ipynb notebook contains the code used to make the machine learning model
*   The SentimentAnalysis.ipynb notebook contains the code used the make the sentiment analysis
*   The imdb_tvshow.csv file contains the data used to run the previous notebooks. It holds rating and review information on 10 TV Shows.
